...
#include "CglTreeProbingInfo.hpp"
...
/*
  jjf: Take action if cut generator can fix a variable (toValue -1 for down,
       +1 for up)

  `Take action' here simply means to save the implication. Only works with
  binary variables.
*/
bool
...
/*
  Based on the state of the code associated with CglTreeProbingInfo, I think
  John's comment here should be interpreted as `this capability is turned
  off as I've discovered that backward is not in the correct order.  FIX this.
*/
...
/*
  jjf: Initializes fixing arrays etc - returns true if we want to save info

  Deletes existing implication arrays and initialises an array of binary
  variable indices and a cross reference.
*/
int
CglTreeProbingInfo::initializeFixing(const OsiSolverInterface * model) 
{
  /*
    Just return if the arrays are already initialised.

    The magic number -2 indicates that convert() has run and done whatever
    it's supposed to do.
  */
  if (numberEntries_ >= 0)
    return (2) ;
  ...
  /*
    ...
    Written to avoid getNumIntegers()? Prior to it? -- lh, 101126 --
  */
  integerVariable_ = new int [numberVariables_] ;
  backward_ = new int [numberVariables_] ;
  numberIntegers_ = 0 ;
  int i ;
  /*
    Set up cross reference for binary variables (type 1). Continuous (0) get
    code -1, general integer (2) get code -2.
  */
  const char *columnType = model->getColType(true) ;
  for (i = 0 ; i < numberVariables_ ; i++) {
    backward_[i] = -1 ;
