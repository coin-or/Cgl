...
void CglRedSplit::generateCuts(OsiCuts &cs)
{
  ...
  /* Every nonbasic variable is shifted below by the bound that its status
     names, and the rhs of each tableau row is taken to be the value of the
     basic variable at xlp. Both steps are only valid if each nonbasic
...
#ifndef NDEBUG
      printf("### WARNING: CglRedSplit::generateCuts(): cstat[%d]: %d but xlp: %.12g, bound: %.12g\n",
	     i, cstat[i], xlp[i], bnd);
#endif
      goodModel = false;
    }
  ...
  for(i=0; (i<nrow) && goodModel; i++) {
    /* rstat describes the slack, so a slack at its lower bound means the row
       is at its upper bound, and vice versa. */
    double bnd;
    if(rstat[i] == 2) {
      bnd = rowLower[i];
    }
    ...
    if(fabs(rowActivity[i] - bnd) > eps_bnd) {
#ifndef NDEBUG
      printf("### WARNING: CglRedSplit::generateCuts(): rstat[%d]: %d but activity: %.12g, bound: %.12g\n",
	     i, rstat[i], rowActivity[i], bnd);
#endif
      goodModel = false;
    }
  }
