			...
			const CglTreeInfo info)
{
   int i ;
/*
  Grab a value for the minimum acceptable violation, if not already set by
  the client.
*/
...
/*
  Consider only the original constraints, or all constraints?
*/
...
/*
  Select the columns of interest, then the rows of interest.  Variables of
  interest x<j> are fractional binary variables. Rows of interest have
  the form SUM{j} x<j> + (stuff with positive coefficients) <= 1 or = 1.

  If the user has asserted that this is a set packing problem, all variables
  are assumed to be binary and all rows are assumed to be cliques.
*/
  ...
  // jjf: Just original rows
  /*
    We just did this! Change to an assert to check.  -- lh, 110222 --
  */
  if (justOriginalRows_ && info.inTree)
     // sp_numrows = CoinMin(info.formulation_rows,sp_numrows) ;
     assert(sp_numrows <= info.formulation_rows) ;
/*
  Create a set-packing submatrix: Just the fractional binary variables.
  
  Then create a column-column (node-node) incidence matrix. The entry for
  columns i,j is true iff columns i and j have a row in common (i.e., both
  variables are present in some constraint).

  Finally, create fgraph (fractional graph). One node for each column with an
  edge if two columns are incident in the node-node matrix. A similar
  organisation to a packed matrix: nodes[i].nbrs points to the start of the
  block of neighbour indices in all_nbrs.
*/
...
/*
  Restore to unset state if not set at start.
*/
   if (!has_petol_set)
      petol = -1.0 ;
}
...
CglClique::find_rcl(OsiCuts& cs)
{
   ...
   for (j = 0; j < sp_numrows; j++) {
	 ...
	 Notice that because node_node[k,k] = false, the columns in the row
	 will not be included in the set of candidates. */
 ...
 * than the threshold  scl_candidate_length_threshold, otherwise attempt to
...
CglClique::greedy_maximal_clique(OsiCuts& cs)
{
   ...
   int cnt = 1 ;
   /*
     Candidates are adjacent to every variable in the row, but not necessarily
     adjacent to one another. Add candidates until we run across one that is
     not adjacent to some candidate we've already added.
   */
   ...
   /*
     Recall that the columns in the row are not included in the set of
     candidates.
   */
   for (j = 0; j < cl_perm_length; j++)
      coef[cnt++] = cl_perm_indices[j] ;
